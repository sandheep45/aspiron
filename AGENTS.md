...
- **Backend endpoints:** `GET /api/v1/topics/:topicId/practice/overview`, `/practice/questions`, `/practice/tests`, `/practice/signals`, `/practice/analytics`
  - Routes in `apps/backend/src/http/routes/content.rs` (under `/api/v1/`, no `/content/` prefix)
  - Handlers in `apps/backend/src/http/handlers/practice_tests.rs` (queries existing tables directly via AppState db)
  - Response DTOs in `apps/backend/src/http/responses/practice_tests.rs` (includes `SignalType` enum)
  - Request payloads in `apps/backend/src/http/payloads/practice_tests.rs` (`QuestionsQueryParams`)
- **API client:** `practiceTestsService` in `packages/api-client/src/services/admin/practice-tests.service.ts`
- **TanStack hooks:** `usePracticeOverviewQuery`, `useQuestionsQuery`, `useTopicTestsQuery`, `usePracticeSignalsQuery`, `useTestAnalyticsQuery` in `packages/tanstack-client/src/hooks/admin/practice-tests.ts`
- **Query keys:** `queryKeys.practiceTests.*` in `packages/tanstack-client/src/types/query-keys.ts`
- **Frontend:** `PracticeTestsPage` at `apps/web-admin/src/features/practice-tests/components/` with 10 sub-components (PracticeOverviewCard, QuestionsTable, DifficultyBadge, QuestionStatusBadge, TopicTestCard, QualitySignalsSection, InsightCard, QuickActionsBar, LoadingSkeleton)
- **Route URL:** `/content/topic/$id/practice-tests` — route at `apps/web-admin/src/routes/_private-routes/content/_content-layout/topic/$id/practice-tests.tsx`
- **Page props:** `topicId`, `onBack` — no routing built in, designed as a composable component
- **Charts:** Recharts `LineChart` / `BarChart` for 4 analytics cards (Score Trend, Attempts Trend, Difficulty Distribution, Question Performance)
- **Visual hierarchy:** Practice Questions (table) → Topic Tests (cards) → Quality Signals → Quick Actions → Analytics (optional)
