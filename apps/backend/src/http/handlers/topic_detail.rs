...
pub async fn handler_get_topic_overview(
    ...
    let recall_strength = if has_recall_sessions {
    ...
    let practice_accuracy = if !quiz_ids.is_empty() {
        ...
        let total_possible: i32 = assessment_quiz::Entity::find()
            .filter(assessment_quiz::Column::Id.is_in(quiz_ids))
            .all(db)
            .await
            .unwrap_or_default()
            .len() as i32
    ...
    let dropoff_indicator = if total_sessions > 0 {
    ...
    let engagement_trend = if recent_progress_count > 5 {
        "growing".to_string()
    } else if recent_progress_count > 0 {
        "stable".to_string()
    } else {
        "declining".to_string()
    };
...
pub async fn handler_get_topic_issues(
    ...
    if has_recall_sessions {
        ...
        if recall_ratio < 0.4 {
            issues.push(TopicIssueResponse {
        ...
        if completion_rate < 0.3 {
            issues.push(TopicIssueResponse {
                ...
                recommendation: "Break content into shorter segments and add engagement checkpoints".to_string(),
        ...
        if attempt_count > 0 && total_possible > 0 {
            ...
            if avg_accuracy < 50.0 {
                issues.push(TopicIssueResponse {
                    ...
                    recommendation: "Review question difficulty and provide step-by-step solution walkthroughs".to_string(),
                    action_label: "View Practice Data".to_string(),
                });
    ...
    if issues.is_empty() {
        issues.push(TopicIssueResponse {
            ...
            recommendation: "Continue monitoring and consider adding enrichment content"
                .to_string(),
            action_label: "Schedule Revision".to_string(),
        });
    }
