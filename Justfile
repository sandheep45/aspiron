...
setup:
    ...
    if [ ! -f local.aspiron.test.pem ] || [ ! -f local.aspiron.test-key.pem ]; then
        echo "Generating SSL certificates..."
        openssl req -x509 -newkey rsa:4096 -keyout local.aspiron.test-key.pem -out local.aspiron.test.pem \
            -days 365 -nodes -subj "/CN=local.aspiron.test" \
            -addext "subjectAltName=DNS:local.aspiron.test,DNS:localhost,IP:127.0.0.1"
...
build-packages:
    pnpm --filter @aspiron/config build
...
generate-types:
    ...
    mkdir -p packages/api-client/src/generated-types
    cp -r apps/backend/bindings/* packages/api-client/src/generated-types/
    rm -rf apps/backend/bindings

    echo "🔧 Fixing ErrorCode enum for Biome compatibility..."

    ERROR_FILE="packages/api-client/src/generated-types/ErrorCode.ts"

    if [ -f "$ERROR_FILE" ]; then
      MEMBERS=$(sed -n '/export enum ErrorCode {/,/}/p' "$ERROR_FILE" \
        | sed '1d;$d' \
        | sed 's/[",]//g' \
        | sed 's/^ *//g' \
        | sed '/^$/d')
      ...
      {
        echo "// This file was generated by [ts-rs](https://github.com/Aleph-Alpha/ts-rs)."
        echo "// Modified by generate-types to replace enum with const object (Biome-compatible). DO NOT EDIT."
        echo ""
        echo "export const ErrorCode = {"
        for m in $MEMBERS; do
          echo "  $m: \"$m\","
        done
        echo "} as const;"
        echo ""
        echo "export type TErrorCode = typeof ErrorCode[keyof typeof ErrorCode];"
      } > "$ERROR_FILE"
    fi
    ...
    for f in *.ts; do \
        name=$(basename "$f" .ts); \
        if [ "$name" == "ErrorCode" ]; then \
            echo "export { $name } from '@/generated-types/$name'" >> index.ts; \
        fi; \
        if [ "$name" != "index" ] && [ "$name" != "uuid" ] && [ "$name" != "ErrorCode" ]; then \
